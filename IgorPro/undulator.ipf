...
// Hideki NAKAJIMA (c) 2024.01.18, rev. 2024.01.31
...
Function Undulator() : Panel
	...
	PopupMenu setPlotOther pos={200,315},size={150,30},title="Others:",value="Phase error;Flux(n),eV;AFD,eV;Bri,eV;F(K,n),K;F(K,n),eV;Q(K,n),K;G(K)",proc=set_plot_other
...
Function B_un_plot()
	...
	if (V_flag == 1)
		...
		TextBox/C/N=topGraphName/F=0/A=LT targetName(2)+" ("+targetName(3)+")\r"+num2str(E_e)+" GeV, "+num2str(I_e*1000)+" mA, "+num2str(u_length/1000)+" m, "+"har. "+num2str(n_har)
...
Function calc_1d(lu, K)
	...
	Do
		...
		Make/D/N=(K_pnt)/O $la, $sr, $sd, $eaf, $ebr, $emr
		Wave w_la = $la, w_sr = $sr, w_sd = $sd, w_eaf = $eaf, w_ebr = $ebr, w_emr = $emr
		...
		SetScale/I	x K0,K,"K", w_fk, w_af, w_qk, w_bt, w_br, w_la, w_sr, w_sd, w_eaf, w_ebr, w_emr
		i = i + 1
	while(i < n_pnt)
...
Function calc_1d_plot()
	...
	String w = "I_1d", harstr, fl, af, fk, en, qk, gk, br, ebr
	...
	elseif (stringmatch(I_1d_plot,"FKneV")==1)
		...
		if (stringmatch(I_1d_plot,"FKn")==1 || stringmatch(I_1d_plot,"FKneV")==1)
			Label/W=$I_1d_plot left "F(K,n)"
		elseif (stringmatch(I_1d_plot,"QKn")==1)
